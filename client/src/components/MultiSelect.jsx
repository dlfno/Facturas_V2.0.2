...
// Dropdown genérico multi-selección con checkboxes.
// Items: [{ key: string, label: string }]. Selected: string[] de keys.
export default function MultiSelect({
  ...
  const filtered = searchable
    ? items.filter((c) => c.label.toLowerCase().includes(search.toLowerCase()))
    : items;
  ...
  const buttonLabel =
    selected.length === 0
      ? allLabel
      : selected.length === 1
      ? (items.find((c) => c.key === selected[0])?.label ?? `1 ${itemLabelSingular}`)
      ...
      {open && (
        <div className={`absolute right-0 top-full mt-1 ${width} bg-white border border-gray-200 rounded-xl shadow-xl z-50`}>
          ...
          <div className="flex items-center justify-between px-3 py-2 border-b border-gray-100">
            <button
              onClick={() => onChange(allSelected ? [] : items.map((c) => c.key))}
